...
describe('AuthController', () => {
  ...
  describe('signIn', () => {
    it('should be logged in', async () => {
      ...
      jest.spyOn(service, 'signIn').mockResolvedValueOnce({ token: 'token', message: 'Authentication Successfully!' });

      expect(await controller.signIn({
        email: ENTITY_USER_COMPLETE_FIXTURE.email,
  ...
  describe('findOne', () => {
    it('should be able to find user', async () => {
      ...
      expect(
          await controller.findOne(ENTITY_USER_COMPLETE_FIXTURE, ENTITY_USER_COMPLETE_FIXTURE.id)
      ).toEqual({
        id: ENTITY_USER_COMPLETE_FIXTURE.id,
        cpf: ENTITY_USER_COMPLETE_FIXTURE.cpf,
        role: ENTITY_USER_COMPLETE_FIXTURE.role,
